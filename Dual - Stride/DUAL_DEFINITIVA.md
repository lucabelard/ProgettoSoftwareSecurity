...
Questo documento presenta l'analisi di sicurezza completa utilizzando la metodologia \textbf{DUAL-STRIDE-DUA}. A seguire troverai la tabella riassuntiva visuale (Landscape), seguita dai dettagli approfonditi per ogni asset (Portrait).

\subsection{Validazione Formale}
\textbf{IMPORTANTE:} L'efficacia delle contromisure di sicurezza documentate in questo documento è stata \textbf{formalmente verificata} tramite model checking PRISM utilizzando Discrete-Time Markov Chains (DTMC). I risultati quantitativi della validazione formale sono disponibili nel documento \texttt{MARKOV\_CHAIN\_ANALYSIS.md}.

\textbf{Risultati chiave della verifica formale:}
\begin{itemize}
\item \textbf{Safety Property:} 100\% (sistema con contromisure) vs 0.0000149\% (sistema vulnerabile)
\item \textbf{Recovery Property:} 97\% entro 20 step (sistema con contromisure) vs 43.5\% (sistema vulnerabile)
\item \textbf{Efficacia contromisure anti-Spoofing (S2.1):} 100\% - attacchi bloccati completamente
\item \textbf{Efficacia contromisure anti-Tampering (T2.1):} 100\% - attacchi bloccati completamente
\end{itemize}

\subsection{Active Defense System}
Il sistema implementa un meccanismo di difesa attiva a tre livelli:
\begin{enumerate}
\item \textbf{IDS (Intrusion Detection System):} Rileva tentativi di attacco in tempo reale monitorando le transazioni e le evidenze IoT
\item \textbf{Rate Limiting:} Contatore di tentativi falliti di autenticazione/validazione (massimo 3 tentativi)
\item \textbf{System Lock:} Dopo 3 tentativi bloccati consecutivi, il sensore entra in stato LOCKED permanente con protezione massima ("blindato" contro ulteriori attacchi)
\end{enumerate}

\subsection{Sensor Redundancy e Auto-Failover}
La ridondanza dei sensori implementa un meccanismo di \textbf{Auto-Failover} con:
\begin{itemize}
\item \textbf{Tasso di successo recovery:} 95\% (vs 60\% senza contromisure)
\item \textbf{Tempo medio di recovery:} <20 step temporali
\item \textbf{Verifica formale:} 97\% probabilità di recovery completo entro 20 step (proprietà Guarantee/Response PRISM)
\end{itemize}
...
\hline
\rowcolor{stripeblue}
A1 & Critico & & & & & & \chk & & & & \chk & E1.1: privilege escalation admin & Multi-sig Timelock \\
\hline
...
\hline
A3 & Critico & & & & & & \chk & & & & \chk & E3.1: drenaggio fondi contratto & Access control withdraw \\
\hline
...
\hline
\textbf{Comments} & \textbf{CAPEC-151:} Identity Spoofing \newline \textbf{Contromisure Implementate:} TPM + Mutual TLS + Whitelist sensori \newline \textbf{Efficacia Verificata:} 100\% (0\% compromissione) \newline \textbf{PRISM Verification:} Safety = 1.0 \newline \textbf{Riferimento:} MARKOV\_CHAIN\_ANALYSIS.md - Sezione 5.1 \\
\hline
\end{longtable}
...
\hline
\textbf{Case Name} & \textbf{errore Indirizzo Destinatario (User Mistake)} \\
\hline
\textbf{Actors} & mittente distratto/inesperto \\
\hline
\textbf{Description} & utente legittimo inserisce indirizzo errato (typo) come parametro mittente durante creazione spedizione, causando pagamento irrecuperabile a indirizzo sbagliato o inesistente. \\
\hline
\textbf{Data} & indirizzo mittente errato, fondi ETH \\
\hline
\textbf{Stimulus/Precond.} & - Utente crea spedizione via DApp \newline - Copy-paste parziale indirizzo \newline - Nessuna validazione UI checksum \\
\hline
\textbf{Basic Flow} & 1. utente chiama \texttt{creaSpedizione(wrongAddress, ...)} \newline 2. Deposita ETH nel contratto \newline 3. Spedizione completata correttamente \newline 4. \texttt{validaEPaga()} trasferisce fondi a wrongAddress \newline 5. Mittente legittimo non riceve pagamento \\
\hline
\textbf{Alternative Flow} & - Se wrongAddress è EOA controllato da terzi: fondi persi \newline - Se wrongAddress è contratto senza fallback: fondi bloccati \\
\hline
\textbf{Exception Flow} & - Validazione checksum EIP-55 in UI previene typo \newline - Conferma doppia indirizzo \\
\hline
\textbf{Response/Post.} & \textbf{Impatto:} perdita irrevocabile fondi, impossibile chargeback blockchain \newline \textbf{NFR:} UX design con validazione indirizzo, warning pre-transazione \\
\hline
\textbf{Comments} & \textbf{DUA-Exposure:} assenza meccanismi protezione utente \newline \textbf{Mitigation:} Address book, QR code, conferma visuale indirizzo \\
\hline
\end{longtable}
...
\item Multi-sig deployment (2/3)
\item Timelock upgrade (48h)
\item Bug bounty program
\end{itemize}

\subsection*{A2 - Evidenze IoT}
\begin{itemize}
\item Firma digitale ECDSA per ogni evidenza
\item Challenge-response authentication sensori
\item Rate limiting invii (max 1 evidenza/sensore/ora)
\item Ridondanza cross-validation (E1 vs E3 temperatura)
...
\item Pull payment pattern (withdrawal invece di transfer)
...
\item Multi-sig per grant/revoke RUOLO\_ORACOLO
\item Timelock ruoli critici
...
\begin{itemize}
\item Private visibility + no getter pubblici
\item Multi-sig 2/3 per modifica CPT
\item Range validation (1-100)
\item Staging test pre-produzione
\item Hashing parametri sensibili
\end{itemize}
...
A4 & HIGH & Multi-sig + Events & LOW & $\checkmark$ Accepted \\
A5 & HIGH & Private + Multi-sig & MEDIUM & $\checkmark$ Accepted \\
...
\newpage
\section{Validazione Formale tramite PRISM}

L'efficacia delle contromisure di sicurezza è stata verificata formalmente tramite model checking \textbf{PRISM} con Discrete-Time Markov Chains (DTMC), modellando il comportamento probabilistico del sistema di sensori IoT sotto attacco.

\textbf{Risultati chiave della verifica formale:}

\begin{itemize}
\item \textbf{Safety Property (compromissione sistema):}
  \begin{itemize}
  \item SENZA contromisure: 0.0000149\% (sistema quasi certamente compromesso)
  \item CON contromisure: \textbf{100\%} (compromissione matematicamente impossibile)
  \item Miglioramento: \textbf{+99.9999851\%}
  \end{itemize}

\item \textbf{Recovery Property (ripristino entro 20 step):}
  \begin{itemize}
  \item SENZA contromisure: 43.5\% (recovery lento e incerto)
  \item CON contromisure: \textbf{97\%} (recovery rapido e affidabile)
  \item Miglioramento: \textbf{+53.5\%} grazie ad Auto-Failover (95\% success rate)
  \end{itemize}
\end{itemize}

\textbf{Conclusione:} Le contromisure TPM, Mutual TLS, Sensor Redundancy e Active Defense bloccano al 100\% gli attacchi di Spoofing/Tampering, rendendo lo stato COMPROMISED formalmente irraggiungibile.

\textbf{Riferimento completo:} Per l'analisi dettagliata, matrici di transizione, diagrammi di stato e codice PRISM, consultare \texttt{prism/MARKOV\_CHAIN\_ANALYSIS.md}.
...
\item [\textcolor{red}{$\bullet$}] \textbf{TODO:} Multi-sig per modifica CPT
\item [\textcolor{red}{$\bullet$}] \textbf{TODO:} HSM per chiavi sensori
\item [\textcolor{orange}{$\bullet$}] \textbf{Consigliato:} zkSNARK per privacy CPT
\end{enumerate}
