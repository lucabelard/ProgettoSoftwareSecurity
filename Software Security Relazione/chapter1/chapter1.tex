...
Per comprendere appieno le dinamiche organizzative e di sicurezza, abbiamo adottato il framework \textbf{i* (iStar)}, che permette di modellare non solo gli aspetti funzionali ma anche le intenzioni, le dipendenze strategiche e le vulnerabilità sociali del sistema.
...
Il softgoal "Protezione finanziaria" del Mittente è quindi costantemente a rischio, dipendendo da una fiducia cieca verso l'operato logistico. Non esiste un meccanismo di *enforcement* automatico per eventuali rimborsi in caso di non conformità ("Ottenere rimborso per non-conformità").
...
\begin{itemize}
    \item \textbf{Validazione Evidenze} e \textbf{Pagamento Automatico}: Il Mittente delega al contratto la verifica dei dati e l'esecuzione condizionale del pagamento.
    \item \textbf{Tracciabilità Immutabile}: Un softgoal soddisfatto intrinsecamente dalla tecnologia blockchain.
    \item \textbf{Rimborso Automatico}: In caso di violazione delle condizioni contrattuali, il Sistema garantisce il ritorno dei fondi depositati senza necessità di arbitrato esterno.
\end{itemize}
...
Infine, l'attore \textit{Admin} gestisce la governance tecnica, occupandosi della "Configurazione Sistema" (es. parametri bayesiani) e della "Gestione Ruoli", ricevendo in cambio dal sistema la garanzia di "Dati Affidabili".
...
L'\textbf{Attaccante Interno} è modellato come un amministratore infedele o compromesso. Il suo goal "Abuso privilegi" si raffina in attacchi specifici come la "Manipolazione CPT" (T5.1-A). Avendo accesso legittimo alle funzioni di configurazione, può alterare le probabilità della rete bayesiana ("Eseguire impostaCPT()") per favorire falsi positivi o negativi, sabotando l'affidabilità del sistema dall'interno senza violare controlli crittografici esterni.

L'\textbf{Attaccante Esterno} punta a "Compromettere sistema" sfruttando vulnerabilità tecniche e umane. I suoi vettori di attacco principali includono:
\begin{itemize}
    \item \textit{Spoofing Sensore} (S2.1-A): Tenta di iniettare dati falsi intercettando o replicando le comunicazioni dei dispositivi IoT ("Replay dati vecchi", "Iniettare dati falsi").
