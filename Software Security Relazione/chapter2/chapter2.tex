...
\begin{lstlisting}[caption={Transizioni PRISM (Principali) dallo stato InAttesa}, label={lst:prism_transitions}, language=C]
// 1. Dinamica Evidenze (Rimanendo in stato 0)
...
// 2. Transizione a PAGATA (1): Validazione OK
...
// 3. Transizione ad ANNULLATA (2): Azione volontaria
[] stato=0 & !evidenze_complete -> (stato'=2);

// 4. Transizione a RIMBORSATA (3): Timeout o Max Tentativi
[] stato=0 & (tempo>=TIMEOUT_HOURS | tentativi_falliti>=MAX_TENTATIVI) ->
    (stato'=3);
\end{lstlisting}
...
\begin{itemize}
    \item \textit{Pagata (1)}: successo della transazione. Secondo le stime operative, questo stato ha una probabilità di occorrenza del \textbf{60\%}.
    \item \textit{Annullata (2)}: annullamento volontario pre-spedizione. Probabilità stimata: \textbf{5\%}.
...
Si nota che per le righe $i \in \{1,2,3\}$, $p_{ii}=1$. Questi sono identificati come \textit{stati ricorrenti} (e assorbenti): una volta raggiunti, il sistema vi permane indefinitamente.
...
\begin{enumerate}
    ...
    \item \textit{Certezza della terminazione}: la prima componente nulla ($\pi_0 = 0$) dimostra che il protocollo non soffre di \textit{deadlock} o \textit{livelock} nello stato intermedio.
    \item \textit{Garanzia di safety}: nel 35\% dei casi critici (stato 3), il sistema evolve correttamente verso lo stato di rimborso, garantendo che i fondi non rimangano mai bloccati nel contratto (property of \textit{fund safety}).
...
La verifica formale è stata condotta specificando i requisiti di sicurezza attraverso la logica temporale \textit{Probabilistic Computation Tree Logic (PCTL)}.

\subsubsection{Analisi di Safety: Integrità Finanziaria}
In prima istanza, l'analisi si è concentrata sulle proprietà di \textit{Safety}, ovvero quelle condizioni indispensabili che il sistema deve sempre rispettare.
...
I risultati della verifica hanno confermato con probabilità \textit{1.0 (Certezza Assoluta)} tutte le proprietà.

\subsubsection{Analisi di Guarantee: Resilienza e Rimborsi}
Successivamente, l'indagine si è spostata sulle proprietà di \textit{Guarantee} (o Liveness). Interrogando il model checker per la probabilità di raggiungere lo stato di rimborso:

\begin{lstlisting}[caption={Proprietà PCTL di Guarantee}, label={lst:pctl_guarantee}, language=C]
// G2: Refund Probability check
P=? [ F stato=3 ]
\end{lstlisting}

PRISM restituisce il valore \texttt{0.35}, confermando esattamente il calcolo algebrico della distribuzione limite effettuato nella sezione precedente. Questo valida la capacità del sistema di gestire gli scenari di errore (timeout, invalidità merce) proteggendo i fondi degli utenti in oltre un terzo dei casi operativi simulati.\subsection{Verifica delle Proprietà PCTL}
...
Il monitor di Safety è modellato come un \textit{Truncation Automaton}. Dallo stato iniziale $q_0$, l'evento critico \texttt{req\_pay} viene intercettato e bufferizzato (\texttt{store}), portando il sistema nello stato di verifica $q_1$. Qui il monitor valuta la condizione di conformità $P$: se $P \ge 95\%$, l'azione viene rilasciata (\texttt{dump}), altrimenti viene soppressa definitivamente (\texttt{halt}) transizionando nello stato di violazione $q_{Err}$.
