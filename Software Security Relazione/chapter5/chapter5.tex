...
Il modello è dichiarato come DTMC nel Listato~\ref{lst:vuln-dtmc-decl}:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Dichiarazione DTMC del modello vulnerabile}, label=lst:vuln-dtmc-decl]
dtmc
\end{lstlisting}
...
Il Listato~\ref{lst:vuln-vars} mostra le variabili di stato del modello vulnerabile:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Variabili di stato del modello vulnerabile}, label=lst:vuln-vars]
...
La Tabella~\ref{tab:matrice-vulnerabile} mostra le probabilità di transizione per un singolo sensore senza contromisure:

\begin{table}[H]
\centering
\label{tab:matrice-vulnerabile}
...
\subsubsection{Sensore OK \texorpdfstring{$\to$}{->} OK, FAILED, o COMPROMISED}
Il Listato~\ref{lst:vuln-ok-trans} mostra la regola di transizione per un sensore nello stato OK:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Transizione OK nel modello vulnerabile}, label=lst:vuln-ok-trans]
    [] e1=0 & time<200 -> 
...
\subsubsection{Sensore FAILED \texorpdfstring{$\to$}{->} OK, FAILED, o COMPROMISED}
Il Listato~\ref{lst:vuln-fail-trans} mostra la transizione dallo stato FAILED:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Transizione FAILED nel modello vulnerabile}, label=lst:vuln-fail-trans]
    [] e1=1 & time<200 -> 
...
\subsubsection{Sensore COMPROMISED \texorpdfstring{$\to$}{->} COMPROMISED (Stato Assorbente)}
Il Listato~\ref{lst:vuln-comp-trans} mostra lo stato assorbente COMPROMISED:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Stato assorbente COMPROMISED (vulnerabile)}, label=lst:vuln-comp-trans]
    [] e1=2 & time<200 -> 
...
Il Listato~\ref{lst:vuln-formule} riporta le formule ausiliarie per classificare lo stato aggregato del sistema:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Formule derivate (modello vulnerabile)}, label=lst:vuln-formule]
...
Il Listato~\ref{lst:vuln-safety} riporta la formula PCTL per la proprietà di Safety S1:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Proprietà Safety S1 (vulnerabile)}, label=lst:vuln-safety]
...
\subsubsection{Codice PCTL}
Il Listato~\ref{lst:vuln-guarantee} riporta la formula PCTL per la proprietà di Guarantee G1:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Proprietà Guarantee G1 (vulnerabile)}, label=lst:vuln-guarantee]
...
\subsubsection{Dichiarazione del Tipo di Modello}
Il Listato~\ref{lst:prot-dtmc-decl} mostra la dichiarazione del modello protetto:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Dichiarazione DTMC del modello protetto}, label=lst:prot-dtmc-decl]
dtmc
\end{lstlisting}
...
Il Listato~\ref{lst:prot-e1-vars} definisce le variabili estese del sensore E1:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Variabili di stato del sensore E1 con Active Defense}, label=lst:prot-e1-vars]
...
Il Listato~\ref{lst:prot-other-vars} definisce i sensori E2-E5 e il contatore temporale:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Sensori E2-E5 e contatore temporale}, label=lst:prot-other-vars]
    e2 : [0..2] init 0;
    e3 : [0..2] init 0;
    e4 : [0..2] init 0;
    e5 : [0..2] init 0;
...
La Tabella~\ref{tab:matrice-protetto} mostra le probabilità di transizione tra gli stati per un sensore con contromisure attive (semplificando il modello senza considerare esplicitamente lo stato LOCKED):

\begin{table}[H]
\centering
\label{tab:matrice-protetto}
\begin{tabularx}{\textwidth}{|l|X|X|X|}
\hline
\textbf{Da Stato $\downarrow$ / A Stato $\rightarrow$} & \textbf{OK (0)} & \textbf{FAILED (1)} & \textbf{COMPROMISED (2)} \\ \hline
...
Il Listato~\ref{lst:prot-ok-trans} mostra la transizione per un sensore OK non bloccato:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={CASO 1: sensore OK non bloccato (protetto)}, label=lst:prot-ok-trans]
...
Il Listato~\ref{lst:prot-lock-trans} mostra l'attivazione del blocco di sicurezza:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={CASO 2: attivazione System Lock}, label=lst:prot-lock-trans]
    [] e1=0 & !e1_locked & e1_attempts = 3 & time<200 ->
...
Il Listato~\ref{lst:prot-locked-safe} mostra il comportamento del sensore in stato bloccato:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={CASO 3: stato Locked (protetto)}, label=lst:prot-locked-safe]
...
Il Listato~\ref{lst:prot-fail-trans} mostra il meccanismo di Auto-Failover:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={CASO 4: sensore FAILED con Auto-Failover}, label=lst:prot-fail-trans]
...
Il Listato~\ref{lst:prot-comp-trans} mostra lo stato assorbente teorico:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={CASO 5: stato COMPROMISED (irraggiungibile)}, label=lst:prot-comp-trans]
    [] e1=2 & time<200 -> 
...
\subsection{Formule Derivate}
Il Listato~\ref{lst:prot-formule} riporta le formule ausiliarie del modello protetto:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Formule derivate (modello protetto)}, label=lst:prot-formule]
formula num_ok = (e1=0?1:0) + (e2=0?1:0) + (e3=0?1:0) + (e4=0?1:0) + (e5=0?1:0);
formula num_failed = (e1=1?1:0) + (e2=1?1:0) + (e3=1?1:0) + (e4=1?1:0) + (e5=1?1:0);
formula num_compromised = (e1=2?1:0) + (e2=2?1:0) + (e3=2?1:0) + (e4=2?1:0) + (e5=2?1:0);
...
\subsubsection{Codice PCTL}
Il Listato~\ref{lst:prot-safety} riporta la formula per la proprietà di Safety S1 del sistema protetto:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Proprietà Safety S1 (protetto)}, label=lst:prot-safety]
P=? [ G<=100 (e1!=2 & e2!=2 & e3!=2 & e4!=2 & e5!=2) ]
\end{lstlisting}
...
\subsubsection{Codice PCTL}
Il Listato~\ref{lst:prot-guarantee} riporta la formula per la proprietà di Guarantee G1 del sistema protetto:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Proprietà Guarantee G1 (protetto)}, label=lst:prot-guarantee]
P=? [ F<=20 (e1=0 & e2=0 & e3=0 & e4=0 & e5=0) ]
\end{lstlisting}
...
\subsubsection{Codice PCTL}
Il Listato~\ref{lst:prot-active-def} riporta la formula per la verifica dell'Active Defense:
\begin{lstlisting}[style=mystyle, language=Prolog, caption={Verifica Active Defense}, label=lst:prot-active-def]
P=? [ F e1_locked ]
\end{lstlisting}
...
La Tabella~\ref{tab:confronto-matrici} riassume il confronto diretto tra le matrici di transizione dei due modelli:
\begin{table}[H]
\centering
\label{tab:confronto-matrici}
\begin{tabularx}{\textwidth}{|l|X|X|X|}
\hline
...
La Tabella~\ref{tab:confronto-prism} sintetizza i risultati delle verifiche formali per entrambi i modelli:
\begin{table}[H]
\centering
\label{tab:confronto-prism}
